...
from numpy import arccos,array,cos,dot,identity,pi,sin,sqrt,trace,transpose,zeros
...
class Vector:
    ...
    def cutting_angle(self, other, deg=False):
        '''
        Return CUTTING angle between two vectors - always > 0 and < 90
        '''
        n1 = self.norm()
        n2 = other.norm()
        c = abs(self * other)/(n1 * n2)
        # Take care of roundoff errors
        c=min(c,1)
        c=max(-1,c)

        rad = arccos(c)
        if deg: return (rad*180) / pi
        else: return rad
